...
parser.add_argument("--data_size", default=2, type=int, help="Size of the mini-batch")
parser.add_argument("--K", default=4, type=int, help="Total number of augmentations (K), sed only in RelationNet")
...
WORKSPACE = '/yuanProject/XPath/relationnet_ds96_512_two_task_adam'
MODE = 'train'
# CHECKPOINT = '/yuanProject/XPath/pretrained/relationnet_slim_resnet34_seed_1_epoch_300.tar'
# CHECKPOINT = None
CHECKPOINT = '/yuanProject/XPath/relationnet_ds96_512_two_task_adam/epoch_200.tar'
SAVE_EPOCH = 1
VALIDATE_EPOCH = 1
...
def main():
    ...
    if (args.backbone == "resnet8"):
        ...
        feature_extractor = ResNet(BasicBlock, layers=[3, 4, 6, 3], zero_init_residual=False,
                                   ...
                                   norm_layer=None)
        feature_decoder = ResNet_Decoder(inplanes=[256, 256, 128, 32], midplanes=[256, 128, 64, 32], outplanes=[128, 64, 32, 16])
    ...
    train_loader = torch.utils.data.DataLoader(
        dataset, batch_size=args.data_size, shuffle=True,
    ...
    if CHECKPOINT != None:
        ...
        feature_decoder.load(CHECKPOINT)
    ...
    optimizer = Adam([{"params": model.net.parameters(), "lr": 0.001, "weight_decay": 5e-4},
                      ...
                      {"params": feature_decoder.parameters(), "lr": 0.001, "weight_decay": 5e-4},])
    ...
    for epoch in range(args.epoch_start, args.epochs):
        ...
        for i, (he_image, ph_image, he_image_list, ph_image_list, _) in enumerate(train_loader):
            ...
            inter_features_list, predictions, loss, train_y, tot_pairs, tot_positive, tot_negative = model.train_forward(train_x, tot_augmentations, batch_size)
            reconstructions, reconstruction_loss = feature_decoder.train_forward(inter_features_list, train_x)
            # print(
            #     train_x.shape,
            #     predictions.shape,
            #     len(inter_features_list),
            #     inter_features_list[0].shape,
            #     inter_features_list[1].shape,
            #     inter_features_list[2].shape,
            #     inter_features_list[3].shape,
            #     loss, reconstruction_loss
            # )
            he_reconstructions = reconstructions[0:reconstructions.shape[0]//2].cpu().detach().numpy()
            ph_reconstructions = reconstructions[reconstructions.shape[0] // 2:].cpu().detach().numpy()
            he_reconstructions = [(item * np.asarray(HE_STAT[1])[:, None, None] + np.asarray(HE_STAT[0])[:, None, None]) * 255.0 for item in he_reconstructions]
            ph_reconstructions = [(item * np.asarray(PH_STAT[1])[:, None, None] + np.asarray(PH_STAT[0])[:, None, None]) * 255.0 for item in ph_reconstructions]
            letters = string.ascii_lowercase
            name = ''.join(random.choice(letters) for i in range(10))
            work_dir = '/yuanProject/XPath/relationnet_ds96_512_two_task_adam/reconstructed/'
            for k in range(0, len(he_reconstructions)):
                Image.fromarray(he_reconstructions[k].transpose((1, 2, 0)).astype(np.uint8)).save(work_dir + '{}_{}_1.jpg'.format(name, k))
                Image.fromarray(ph_reconstructions[k].transpose((1, 2, 0)).astype(np.uint8)).save(work_dir + '{}_{}_2.jpg'.format(name, k))


            optimizer.zero_grad()
            contrastive_loss_meter.update(loss.item(), len(train_y))
            reconstruction_loss_meter.update(reconstruction_loss.item(), train_x.shape[0])
            total_loss_meter.update((loss+reconstruction_loss).item(), train_x.shape[0])
        ...
        print("Epoch [" + str(epoch) + "]"
              ...
              + " contrastive loss: " + str(contrastive_loss_meter.avg)
              + " reconstruction loss: " + str(reconstruction_loss_meter.avg)
              + " total loss: " + str(total_loss_meter.avg)
        ...
        with open(log_file, "a") as myfile:
                myfile.write(str(epoch)+","+str(contrastive_loss_meter.avg)+","+str(reconstruction_loss_meter.avg)+","+str(total_loss_meter.avg)+","+str(accuracy_meter.avg)+"\n")

        # draw to visdom
        plotter.line_plot(var_name='loss', split_name='contrastive', title_name='Loss', x=epoch, y=contrastive_loss_meter.avg)
        plotter.line_plot(var_name='loss', split_name='reconstruction', title_name='Loss', x=epoch, y=reconstruction_loss_meter.avg)
        plotter.line_plot(var_name='loss', split_name='total', title_name='Loss', x=epoch, y=total_loss_meter.avg)
        ...
        if (epoch+1) % VALIDATE_EPOCH == 0:
            ...
            with torch.no_grad():
                ...
                for i, (he_image, ph_image) in enumerate(test_loader_query):
                    data = torch.cat((he_image, ph_image), 0).to(device)
                    inter_features_list, output = model.test_forward(data)
                    query_he_embeddings_list.append(output[0:output.shape[0] // 2])

                # reconstruct last batch
                original_images = data
                reconstructions = feature_decoder.test_forward(inter_features_list)
            ...
            he_reconstructions = reconstructions[0:reconstructions.shape[0]//2].cpu().detach().numpy()
            ph_reconstructions = reconstructions[reconstructions.shape[0] // 2:].cpu().detach().numpy()
            he_original_data = original_images[0:original_images.shape[0]//2].cpu().detach().numpy()
            ph_original_data = original_images[original_images.shape[0] // 2:].cpu().detach().numpy()
            he_reconstructions = [(item * np.asarray(HE_STAT[1])[:, None, None] + np.asarray(HE_STAT[0])[:, None, None]) * 255.0 for item in he_reconstructions]
            ph_reconstructions = [(item * np.asarray(PH_STAT[1])[:, None, None] + np.asarray(PH_STAT[0])[:, None, None]) * 255.0 for item in ph_reconstructions]
            he_original_data = [(item * np.asarray(HE_STAT[1])[:, None, None] + np.asarray(HE_STAT[0])[:, None, None]) * 255.0 for item in he_original_data]
            ph_original_data = [(item * np.asarray(PH_STAT[1])[:, None, None] + np.asarray(PH_STAT[0])[:, None, None]) * 255.0 for item in ph_original_data]
            ...
            plotter.image_plot(title_name='he_reconstructions_{}'.format(epoch), x=he_reconstructions)
            plotter.image_plot(title_name='ph_reconstructions_{}'.format(epoch), x=ph_reconstructions)
            plotter.image_plot(title_name='he_original_data_{}'.format(epoch), x=he_original_data)
            plotter.image_plot(title_name='ph_original_data_{}'.format(epoch), x=ph_original_data)
